...
/* =========================================================
   STEP 1 — Source Format Tracker
   - يحفظ نسخة من ملف السورس الأصلي في الذاكرة المؤقتة
   - لا يصدّر بعد
   - يمهد لخطوة تصدير Target بنفس صيغة Source
========================================================= */
(function () {
  "use strict";
  ...
  window.__CAT_SOURCE_ORIGINAL__ = window.__CAT_SOURCE_ORIGINAL__ || {
    kind: "",
    name: "",
    size: 0,
    buffer: null,
  ...
  async function rememberOriginalFile(file, sourceLabel) {
    if (!file) return;
    ...
    try {
      ...
      window.__CAT_SOURCE_ORIGINAL__ = {
        kind: kind,
        name: file.name || "",
        size: file.size || 0,
        buffer: buffer,
      ...
      status(
        "تم تسجيل ملف السورس الأصلي: " +
        (file.name || "") +
        " — النوع: " +
        kind.toUpperCase() +
        " — هذه خطوة تمهيدية للتصدير بنفس صيغة السورس."
      );

      console.log("[CAT] Original source file saved:", window.__CAT_SOURCE_ORIGINAL__);
    } catch (e) {
      status("تعذر تسجيل ملف السورس الأصلي: " + (e && e.message ? e.message : e));
    }
  }

  function bindFileInput(sh, inputId, label) {
    var input = sh && sh.getElementById(inputId);
    if (!input || input.getAttribute("data-source-tracker-bound")) return;

    input.setAttribute("data-source-tracker-bound", "1");

    input.addEventListener("change", function () {
      var file = input.files && input.files[0];
      if (!file) return;
      rememberOriginalFile(file, label);
    }, true);
  }
  ...
  function install() {
    ...
    bindFileInput(sh, "fileDOCX", "word-source");
    bindFileInput(sh, "fileExcelTM", "excel-source");
    bindFileInput(sh, "fileHTML", "html-source");
  }
